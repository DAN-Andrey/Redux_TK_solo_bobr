import type { UserSignInData, UserSignUpData } from "../model";
export default class UserValidate {
  ...
  static validateLoginData({ email, password }: UserSignInData) {
    if (
      !email ||
      typeof email !== "string" ||
      email.trim().length === 0 ||
  ...
  static validateSignUpData({ name, email, password }: UserSignUpData) {
